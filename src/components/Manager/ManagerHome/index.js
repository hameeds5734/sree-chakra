...
const ViewBranch = () => {
  ...
  const searchBill = () => {
    setFileName('SCSAppServiceBills'+ new Date())
      ...
      axios.post(AllServices.SEARCH_BRANCH_BILLS(), { branch_id: user.branch_id ,date1:date1, date2:date2, searchText:searchText, searchType:searchType }).then(response=>{
      ...
      axios.get(AllServices.PENDING_BILLS_COUNT(user.branch_id))
      .then(response=>{
        if(response.data.status === 200){
      ...
      axios.get(AllServices.RECIEVED_BILLS_COUNT(user.branch_id))
      .then(response=>{
        if(response.data.status === 200){
      ...
      // complete
      axios.get(AllServices.COMPLETED_BILLS_COUNT(user.branch_id))
      .then(response=>{
        if(response.data.status === 200){
  ...
  const Update_Payment = () => {
    ...
    // axios.put(AllServices.UPDATE_PAYMENT_STATUS(currentBill.id), { pay_status:pay_status }).then(response=>{
    //   if(response.data.status === 200){
    //     searchBill()
    //   }
    //   else{
    ...
    useEffect(()=>{
      // box1
      axios.get(AllServices.TOTAL_BILLS_COUNT(user.branch_id))
      .then(response=>{
        if(response.data.status === 200){
      ...
      // box2
      // axios.get(AllServices.MONTH_BILLS_COUNT(user.branch_id))
      // .then(response=>{
      //   if(response.data.status === 200){
      ...
      // })
      // box4
      // axios.get(AllServices.TODAY_BILLS_COUNT(user.branch_id))
      // .then(response=>{
      //   if(response.data.status === 200){
    ...
    <div>
      ...
      >
        <Box sx={style} style={{borderRadius:15,borderColor:'#000',borderWidth:1}}>
        <span style={{textAlign:'center',width:'100%'}}>Bill : <span style={{color:'#000'}}>{currentBill?currentBill.employee_name:null}</span> </span>
        <span style={{textAlign:'center',width:'100%'}}> <b>|</b> Service Persons : <span style={{color:'#000'}}>{currentBill?currentBill.services_persons:null}</span></span>
        <Grid container columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
          <Grid item xs={12} sm={4} md={4} lg={4} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666',}}><span>Invoice ID</span></b>
              <b style={{}}>: <span style={{color:'#000'}}>{currentBill?currentBill.invoice_id:null}</span></b>
          </Grid>
          <Grid item xs={12} sm={4} md={4} lg={4} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666',}}>Customer Name </b>
              <b style={{}}>: <span style={{color:'#000'}}>{currentBill?currentBill.customer_name:null}</span></b>
          </Grid>
          <Grid item xs={12} sm={4} md={4} lg={4} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666',}}>Phone Number </b>
              <b style={{}}>: <span style={{color:'#000'}}>{currentBill?currentBill.customer_phoneno:null}</span></b>
          </Grid>
          <Grid item xs={12} sm={12} md={12} lg={12}>
            <Divider style={{backgroundColor:'#ddd'}} />
          </Grid>
          <Grid item xs={12} sm={6} md={6} lg={6} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666'}}>Customer Address </b>
              <b> &nbsp;&nbsp;&nbsp;: <span style={{color:'#000'}}>{currentBill?currentBill.customer_address:null}</span></b>
          </Grid>
          <Grid item xs={12} sm={6} md={6} lg={6} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666'}}>Service Charge </b>
              <b> &nbsp;&nbsp;&nbsp;: <span style={{color:'#000'}}>{currentBill?currentBill.s_charge:null}</span></b>
          </Grid>
          <Grid item xs={12} sm={12} md={12} lg={12}>
            <Divider style={{backgroundColor:'#ddd'}} />
          </Grid>
          <Grid item xs={12} sm={6} md={6} lg={6} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666'}}>Rectification Details </b>
              <b>: <span style={{color:'#000'}}>{currentBill?currentBill.rectification_details:null}</span></b>
          </Grid>
          <Grid item xs={12} sm={6} md={6} lg={6} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666'}}>Remarks </b>
              <b> &nbsp;&nbsp;&nbsp;: <span style={{color:'#000'}}>{currentBill?currentBill.remarks:null}</span></b>
          </Grid>
          <Grid item xs={12} sm={12} md={12} lg={12}>
            <Divider style={{backgroundColor:'#ddd'}} />
          </Grid>
          <Grid item xs={12} sm={4} md={4} lg={4} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666',}}>Nature of Job </b>
              <b style={{}}>: <span style={{color:'#000'}}>{currentBill?currentBill.nature_of_job:null}</span></b>
          </Grid>
          <Grid item xs={12} sm={4} md={4} lg={4} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666',}}>Bill Type </b>
              <b style={{}}>: <span style={{color:'#000'}}>{currentBill?currentBill.bill_type:null}</span></b>
          </Grid>
          <Grid item xs={12} sm={4} md={4} lg={4} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666',}}>Working condition </b>
              <b style={{}}>: <span style={{color:'#000'}}>{currentBill?currentBill.working_condition:null}</span></b>
          </Grid>

          <Grid item xs={12} sm={4} md={4} lg={4} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666',}}>In Time </b>
              <b style={{}}>: <span style={{color:'#000'}}>{currentBill?currentBill.in_time:null}</span></b>
          </Grid>
          <Grid item xs={12} sm={4} md={4} lg={4} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666',}}>Out Time </b>
              <b style={{}}>: <span style={{color:'#000'}}>{currentBill?currentBill.out_time:null}</span></b>
          </Grid>
          <Grid item xs={12} sm={4} md={4} lg={4} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666',}}>Date </b>
              <b style={{}}>: <span style={{color:'#000'}}>{currentBill?moment(currentBill.bill_date).format('DD-MM-YYYY'):null}</span></b>
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={3} lg={3} style={{display:'flex',paddingTop:20,paddingBottom:20}}></Grid>

            <Grid item xs={12} sm={6} md={3} lg={3} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666',width:'50%'}}><span>Bill Type</span></b>
              <b style={{width:'50%'}}>: <span style={{color:'#000'}}>{currentBill?currentBill.payment_method:null}</span></b>
            </Grid>
            <Grid item xs={12} sm={6} md={3} lg={3}>
                {
                pay_status != "COMPLETED"?
                <SelectBox status={pay_status} setPaymentStatus={setPaymentStatus} />
                :<b style={{color:'green',display:'flex',paddingTop:20,paddingBottom:20}}>PAID&nbsp; <CheckCircleIcon style={{alignSelf:'center',fontSize:16}} /></b>
                }
            </Grid>
            {
            pay_status != "COMPLETED"?
            ...
            :<Grid item xs={12} sm={6} md={3} lg={3} style={{padding:10,alignItems:'center'}} ></Grid>}
            <Grid item xs={12} sm={6} md={3} lg={3} style={{padding:10,alignItems:'center'}} >
              <Button variant="contained" style={{width:'100%',marginTop:6,backgroundColor:'#ccc',color:'#000'}}
              onClick={handleClose}>
                CLOSE
              </Button>
            </Grid>
      ...
      >
        ...
        <Grid container columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
          <Grid item xs={12} sm={4} md={4} lg={4} style={{display:'flex',paddingTop:20,paddingBottom:20}}>
              <b style={{color:'#666',}}><b>Invoice ID</b></b>
        ...
        <Grid item sm={12} xs={12} lg={12}>
          <Grid container>
            <Grid xs={6} md={4} lg={4} item style={{paddingRight:5,marginTop:10}}>
                <BasicDateRangePicker setDate1={setDate1} setDate2={setDate2} />
            </Grid>
            <Grid xs={4} md={3} lg={3} item style={{marginTop:10}}>
